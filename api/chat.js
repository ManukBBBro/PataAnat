...
export default async function handler(req,res){
  ...
  if(!apiKey)return res.status(500).json({error:'ANTHROPIC_API_KEY не задан в Vercel → Settings → Environment Variables'});

  const{system,messages,userId}=req.body||{};
  if(!messages||!userId)return res.status(400).json({error:'Invalid request'});

  let used=0,isPremium=false,dbOk=Boolean(SUPABASE_URL&&SUPABASE_KEY);
  if(dbOk){
    try{
      let user=await dbGet(userId);
      if(!user){await dbCreate(userId);user={messages_used:0,is_premium:false}}
      isPremium=user.is_premium;used=user.messages_used||0;
  ...
  try{
    ...
    if(dbOk){try{await dbInc(userId,used)}catch(e){}}
